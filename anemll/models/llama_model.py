...
class LlamaConfig:
    def __init__(self, **kwargs):
        ...
        if self.rope_scaling:
            self.rope_scaling["rope_type"] = self.rope_scaling.get(
                "rope_type", "llama3"
            )
...
class NA_LayerNormANE(nn.Module):
    ...
    def forward(self, hidden_states):
        input_dtype = hidden_states.dtype
        hidden_states = hidden_states.to(MODEL_DTYPE)
        hidden_states = torch.clamp(hidden_states, min=-1000, max=1000).to(
            MODEL_DTYPE
        )  # ANE tuned value!
        mean = hidden_states.mean(-1, keepdim=True)
        hidden_states = hidden_states - mean
        variance = (
            (hidden_states * hidden_states).mean(-1, keepdim=True).to(MODEL_DTYPE)
        )
        variance = torch.clamp(variance, min=self.eps, max=self.max_clip).to(
            MODEL_DTYPE
        )  # ANE tuned value!
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config):
        ...
        inv_freq = 1.0 / (
            self.base
            ** (torch.arange(0, self.dim, 2).float().to(TEST_DEVICE) / self.dim)
        )
        self.register_buffer("inv_freq", inv_freq)

        # Cache cos and sin values for positions
        t = torch.arange(self.max_position_embeddings, device=TEST_DEVICE).type_as(
            self.inv_freq
        )
    ...
    def rotate(
        ...
        rotated = torch.cat(
            [x1 * cos - x2 * sin, x2 * cos + x1 * sin],  # Real part  # Imaginary part
            dim=-1,
        )
...
class LlamaAttention(nn.Module):
    ...
    def get_new_kv_cache(self, hidden_states, current_pos, rotary_emb):
        ...
        if ENABLE_DEBUG2:
            ...
            print(
                f"  hidden_states: {hidden_states.shape}, current_pos: {current_pos}, q_len: {q_len}"
            )
        ...
        query_states = (
            ...
            .view(1, self.num_heads, 1, self.head_dim)
            .to(MODEL_DTYPE)
        )
        key_states = (
            self.k_proj(hidden_states)
            .view(1, self.num_key_value_heads, 1, self.head_dim)
            .to(MODEL_DTYPE)
        )
        value_states = (
    ...
    def get_new_kv_cache_prefill(
        self, hidden_states, current_pos, rotary_emb, batch_size
    ):
        ...
        if ENABLE_DEBUG3:
            ...
            print(
                f"  hidden_states: {hidden_states.shape}, current_pos: {current_pos}, batch: {batch}"
            )

        # Project QKV and ensure MODEL_DTYPE - optimized for batch processing
        hidden_states = (
            hidden_states.permute(0, 2, 1).unsqueeze(2).to(MODEL_DTYPE)
        )  # [1, hidden_size, 1, batch]
        ...
        key_states = self.k_proj(
            hidden_states
        )  # [1, num_kv_heads * head_dim, 1, batch]
        ...
        query_states = query_states.view(
            ...
            0, 1, 3, 2
        )  # [1, num_heads, batch, head_dim]
        key_states = key_states.view(
            1, self.num_key_value_heads, self.head_dim, batch
        ).permute(
            0, 1, 3, 2
        )  # [1, num_kv_heads, batch, head_dim]
    ...
    def forward(
        ...
        **kwargs,
        ...
        cos, sin = self.rotary_emb(
            hidden_states, seq_len=seq_length, current_pos=current_pos
        )
        ...
        if kwargs.get("kv_cache_layer", None) is not None:
            ...
            if current_pos is not None:
                ...
                print(
                    f"    Update slice - key_cache[:, {current_pos}:{current_pos + seq_length}]"
                )
                ...
                value_states_reshaped = value_states.squeeze(
                    0
                ...
                # Update cache
                key_cache[:, current_pos : current_pos + seq_length] = (
                    key_states_reshaped
                )
                value_cache[:, current_pos : current_pos + seq_length] = (
                    value_states_reshaped
                )

                # Use full cache for attention
                key_states = key_cache[:, : current_pos + seq_length].unsqueeze(
                    0
                )  # Add batch dimension back
                value_states = value_cache[:, : current_pos + seq_length].unsqueeze(
                    0
                )  # Add batch dimension back
    ...
    def forward_regular(
        self, hidden_states, query_states, kv_cache_layer=None, causal_mask=None
    ):
        ...
        if causal_mask is not None:
            attn_weights = (
                attn_weights + causal_mask[:, :, : self.config.context_length]
            )

        if ENABLE_VALUES:
            print(
                f"[TRACE]  SINGLE attn_weights first 10={attn_weights[0, -1, 0:10, 0:10].tolist()}"
            )
    ...
    def forward_prefill(
        ...
        # Compute scaled dot-product attention
        attn_weights = (
            torch.einsum("bhqd,bhkd->bhqk", query_states, key_states)
            * self.scaling_factor
        )
        ...
        if ENABLE_VALUES:
            print(
                f"[forward_prefill]  BATCH attn_weights first 10={attn_weights[0, -1, 0:10, 0:10].tolist()}"
            )

        attn_weights = self.ANE_softmax(attn_weights, dim=-1)
        attn_output = torch.einsum(
            "bhqk,bhkd->bhqd", attn_weights, value_states
        )  # [batch=1, heads=32, q_len=1, head_dim=64]
    ...
    def repeat_kv(self, x: torch.Tensor, n_rep: int) -> torch.Tensor:
        ...
        )  # Shape: (1, num_kv_heads * n_rep, seq_len, head_dim)
        return x
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(self, config, layer_idx: int, use_ane_norm=False):
        ...
        if use_ane_norm:
            self.input_layernorm = LayerNormANE(
                config.hidden_size, eps=config.rms_norm_eps
            )
            self.post_attention_layernorm = LayerNormANE(
                config.hidden_size, eps=config.rms_norm_eps
            )
        else:
...
class LlamaModel(BaseModel):
    ...
    def __init__(self, config, use_ane_norm=False, model_path=None):
        ...
        if FORCE_UNIFIED_CACHE or ENABLE_UNIFIED_CACHE:
            ...
            if ENABLE_DEBUG:
                print(
                    f"Initialized unified KV kv_cache_0 with shape: {self.kv_cache_0.shape}"
                )
        else:
    ...
    def get_rotary_embeddings_s(self, current_pos):
        ...
        sin = (
            self.layers[0]
            .self_attn.rotary_emb.sin_cached[:, current_pos]
            .view(1, 1, 1, -1)
        )
        cos = (
            self.layers[0]
            .self_attn.rotary_emb.cos_cached[:, current_pos]
            .view(1, 1, 1, -1)
        )
        if ENABLE_VALUES:
            print(f"  cos shape: {cos.shape}")
    ...
    def get_rotary_embedding_prefill(self, positions):
        ...
        if ENABLE_DEBUG3:
            ...
            print(
                f"[TRACE] get_rotary_embedding_prefill Batched rotary from pos {positions[0]}:"
            )
    ...
    def process_layer_prefill(
        ...
        if ENABLE_VALUES:
            ...
            print(
                f"Layer {layer_idx} after input_layernorm hidden states (first 16 values):"
            )
            print(f"  {hidden_states[0, 0, :16].tolist()}")
        ...
        query_states, key_states, value_states = (
            layer.self_attn.get_new_kv_cache_prefill(
        ...
        kv_cache[
            key_idx : key_idx + 1, :, current_pos : current_pos + seq_length, :
        ] = key_states
        kv_cache[
            value_idx : value_idx + 1, :, current_pos : current_pos + seq_length, :
        ] = value_states
    ...
    def process_layer_regular(
        ...
        kv_cache[key_idx : key_idx + 1, :, current_pos : current_pos + 1, :] = (
            key_states
        )
        kv_cache[value_idx : value_idx + 1, :, current_pos : current_pos + 1, :] = (
            value_states
        )
    ...
    def forward(
        ...
        if ENABLE_DEBUG2:
            print(
                f"LlamaModel.forward - hidden_states last 10: {hidden_states[-1, -1, -10:].tolist()}"
            )
        ...
        if end_layer is None or end_layer == len(self.layers):
            if IN_PREFILL:
                ...
                if ENABLE_VALUES:
                    print(
                        f"LlamaModel.forward b4 self.norm hidden_states last 10: {hidden_states[-1, -1, -10:].tolist()}"
                    )
                hidden_states = self.norm(hidden_states)
                if ENABLE_VALUES:
                    print(
                        f"LlamaModel.forward AFTER self.norm hiddhistoryen_states last 10: {hidden_states[-1, -1, -10:].tolist()}"
                    )
    ...
    def forward_prefill(
        ...
        if end_layer is None or end_layer == len(self.layers):
            ...
            if ENABLE_VALUES:
                print(
                    f"LlamaModel.forward AFTER self.norm hidden_states last 10: {hidden_states[-1, -1, -10:].tolist()}"
                )
...
class LlamaForCausalLM(nn.Module):
    ...
    def __init__(self, config, use_ane_norm=False, enable_coreml=False):
        ...
        if ENABLE_CONV2D:
            ...
            self.lm_head = nn.Linear(
                config.hidden_size, config.vocab_size, bias=False, dtype=MODEL_DTYPE
            ).to(TEST_DEVICE)
    ...
    def load_pretrained_weights(self, model_path, **kwargs):
                ...
                if ENABLE_CONV2D:
                    ...
                    if ENABLE_VACAB_SPLIT8:
                        ...
                        for i, split in enumerate(splits):
                            ...
                            print(
                                f"Split lm_head weight into lm_head8_{i+1}.weight with shape {split.shape}"
                            )
                    elif ENABLE_VACAB_SPLIT:
                ...
                if "layers." in new_key:
                    ...
                    if "self_attn" in new_key and "weight" in new_key:
                        if "o_proj" in new_key:
                            ...
                            print(
                                f"Keeping o_proj weights as 2D: {new_key} shape {v.shape}"
                            )
                        else:
                            reshaped_weight = v.view(v.shape[0], v.shape[1], 1, 1)
                            base_filtered_dict[new_key] = reshaped_weight
                            print(
                                f"Reshaped {new_key} from {v.shape} to {reshaped_weight.shape}"
                            )
                    # Handle MLP weights
        ...
        missing_keys, unexpected_keys = self.model.load_state_dict(
            base_filtered_dict, strict=False
        )
        ...
        expected_missing.extend(
            [
                f"layers.{i}.self_attn.rotary_emb.inv_freq"
    ...
    def forward(
        ...
        **kwargs,
        ...
        if ENABLE_DEBUG:
            print(
                f"LlamaForCausalLM::forward called with input_ids: {input_ids.shape}, update_mask: {update_mask.shape}, position_ids: {position_ids.shape}, causal_mask: {causal_mask.shape}, current_pos: {current_pos}"
            )
        # Get embeddings
        ...
        ), f"input_ids must be a 2D tensor, got shape {input_ids.shape}"
        if not IN_PREFILL:
            assert (
                len(position_ids.shape) == 1
            ), f"position_ids must be a 1D tensor for Inference, got shape {position_ids.shape}"
        else:
            assert (
                position_ids.shape[-1] == input_ids.shape[-1]
            ), f"position_ids last dimension should match input_ids for Prefill, got shape {position_ids.shape} and input_ids shape {input_ids.shape}"
        ...
        if ENABLE_VALUES:
            print(
                f"LlamaForCausalLM::embed_tokens weight dtype: {self.embed_tokens.weight.dtype}"
            )
            print(
                f"LlamaForCausalLM::embed_tokens input_idss (values):{input_ids.tolist()}"
            )
            print(
                f"LlamaForCausalLM::embed_tokens model input hidden states (first 16 values):{hidden_states[0,0,:16].tolist()}"
            )
...
class LlamaConverter(BaseConverter):
    ...
    def __init__(self, config, model_path=None, use_ane_norm=False):
        ...
        self.model = LlamaForCausalLM(
            config, use_ane_norm=use_ane_norm, enable_coreml=True
        )

        if False and model_path:
            self.model.model.load_pretrained_weights(
                model_path, enable_conv2d=True, mlp_up_split=1, mlp_down_split=1
            )
